...
class MARNNrBaseEncoder(nn.Module):

    def __init__(self, idim, cdim, N, M, dropout, read_first):
        super(MARNNrBaseEncoder, self).__init__()
        self.idim = idim
        self.odim = M
        self.cdim = cdim
        self.N = N
    ...
    def forward(self, **kwargs):
        ...
        for t, emb in enumerate(embs):
            if 'analysis_mode' in dir(self) and self.analysis_mode:
                ...
                i, f, g, o, c_new, h_new = _calc_gates(self, emb[None], h, c)
                line = {'type':'gates',
                        't': t,
            ...
            controller_outp, (h, c) = self.controller(emb[None], (h, c))
